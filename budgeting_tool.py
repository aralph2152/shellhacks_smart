...
def app():
    st.write("<h1 style='font-weight: bold; font-style: italic;'><span style='color: #F4FADA;'"
             ">Budgeting</span> <span style='color: #919A67;'>Tool</span></h1>", unsafe_allow_html = True)
    st.markdown("#### *Try out a new budget and help visualize where your money goes with the budgeting tool*")
    st.divider()

    st.write("""Budgeting is a powerful tool that allows you to plan your finances according to your unique needs and goals.
            A popular budgeting framework is the **50/30/20 rule**, where:""")
    st.write("✦ **50%** of your income goes towards *Needs* (e.g., rent, groceries, bills)")
    st.write("✦ **30%** is allocated to *Wants* (e.g., entertainment, dining out)")
    st.write("✦ **20%** goes towards *Financial Freedom* (e.g., savings, paying off debt)")
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color = '#D3DCAC',
        text = [f"{val:.2f}%" for val in benchmark_percentages],
        textposition = 'auto'
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        name = 'Your Budget',
        marker_color = '#919A67',
        text = [f"{val:.2f}%" for val in user_budget_percentages],
        textposition = 'auto'
    ))
    ...
    st.divider()
    st.markdown("### *Budget Analysis and Recommendations*")
    ...
    if remaining_balance < 0:
        st.markdown(f"✦ You're overspending by ${-remaining_balance:.2f}. Consider reducing spending in some categories.")
    else:
        st.markdown("✦ You're within your budget! Great job!")

    if user_budget_percentages[0] > 50:
        st.markdown("✦ Your spending on Needs exceeds the recommended 50%. Try reducing necessary expenses where possible.")
    if user_budget_percentages[1] > 30:
        st.markdown("✦ Your spending on Wants exceeds the recommended 30%. Consider cutting down on discretionary spending.")
    if user_budget_percentages[2] < 20:
        st.markdown("✦ You're not allocating 20% towards Financial Freedom. Consider increasing your savings or debt repayment.")
    st.divider()